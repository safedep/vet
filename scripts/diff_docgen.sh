...
# Check diff between ./docs/manual and ./temp-dir/ considering some ignore pattens (see bellow)
...
#   Then we can't gurantee the order of these values, so diff may show a change
...
# Since ./docs/manual can have other `github pages` relatde files, we strictly use `vet*.md` regex
