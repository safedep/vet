...
func newScanCommand() *cobra.Command {
	...
	cmd.Flags().BoolVarP(&dependencyUsageEvidence, "dependency-usage-evidence", "", false, "Enable dependency usage evidence during scan")
	cmd.Flags().StringVarP(&codeAnalysisDbPath, "code-db", "", "", "Path to code analysis database generated by 'vet code scan'")
	...
	cmd.PreRunE = func(cmd *cobra.Command, args []string) error {
		err := func() error {
			...
			if dependencyUsageEvidence && codeAnalysisDbPath == "" {
				return fmt.Errorf("code-db is required for dependency usage evidence analysis")
			}
...
func internalStartScan() error {
	...
	if enableCodeAnalysisEnricher() {
		...
		codeAnalysisEnricher := scanner.NewCodeAnalysisEnricher(scanner.CodeAnalysisEnricherConfig{
...
// Enable code analysis enricher if flags that require code analysis are enabled
func enableCodeAnalysisEnricher() bool {
	return dependencyUsageEvidence
}
